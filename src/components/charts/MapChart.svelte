<script>
	...
	$: newList = $orgData
	$: summary = rollup($orgData, v => v.length, d => d.state)
	$: orgExtent = extent(summary, d => d[1])
	...
	let showHide = function(name) {
		let x = document.getElementById(`${name}-description`);
		console.log(x)
	...
	$: filteronState = function (state) {
		newList = $orgData.filter(d => d.state == state)
		return newList
	}
...
<section id="map" class="chart-section">
	<ChartTitle 
		title={title}
		...
		<svg style="{`width: ${width}px; height: ${height}px`}">
			<g class="states">
				{#each data as feature}
					...
					class="stateShape {getColor(feature) != "#d3d3d3" ? "filled" : "not-filled"}"
	...
	<div class="org-list">
	  {#each newList as org}
		<div class="org-list__item" on:click={showHide(org.name)}>
				...
				{/if}
				<p class="org-description" id="{org.name}-description">{org.description}</p>
		</div>
	  {/each}
	</div>
</section>